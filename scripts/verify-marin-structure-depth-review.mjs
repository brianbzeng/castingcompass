...
async function main() {
  ...
  const sources = await loadStructureDepthReviewSources("marin", DEFAULT_ROOT);

  if (command === "verify-policy") {
    requireCondition(args.length === 0, "verify-policy does not accept arguments.");
    process.stdout.write(`${JSON.stringify(validateMarinStructureDepthReview(sources), null, 2)}\n`);
    return;
  }
  if (command === "print-template") {
    ...
    process.stdout.write(`${JSON.stringify(createMarinStructureDepthReviewTemplate({
      ...sources,
      reviewedCommit,
    }), null, 2)}\n`);
    return;
  ...
  if (command === "evaluate") {
    ...
    const receipt = evaluateMarinStructureDepthReview({
      ...sources,
  ...
  throw new Error(
    "Usage: verify-marin-structure-depth-review.mjs verify-policy | print-template --expected-commit <sha> | write-template --output-file <absolute-path> --expected-commit <sha> | evaluate --evidence-file <absolute-path> --expected-commit <sha>",
  );
}
