...
async function main() {
  ...
  const sources = await loadStructureDepthReviewSources("san-francisco", DEFAULT_ROOT);

  if (command === "verify-policy") {
    requireCondition(args.length === 0, "verify-policy does not accept arguments.");
    process.stdout.write(`${JSON.stringify(validateSanFranciscoStructureDepthReview(sources), null, 2)}\n`);
    return;
  }
  if (command === "print-template") {
    ...
    process.stdout.write(`${JSON.stringify(createSanFranciscoStructureDepthReviewTemplate({
      ...sources,
      reviewedCommit,
    }), null, 2)}\n`);
    return;
  ...
  if (command === "evaluate") {
    ...
    const receipt = evaluateSanFranciscoStructureDepthReview({
      ...sources,
  ...
  throw new Error(
    "Usage: verify-san-francisco-structure-depth-review.mjs verify-policy | print-template --expected-commit <sha> | write-template --output-file <absolute-path> --expected-commit <sha> | evaluate --evidence-file <absolute-path> --expected-commit <sha>",
  );
}
