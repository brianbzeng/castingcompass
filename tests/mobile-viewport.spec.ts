...
const SITES_FIXTURE = readFileSync(
  resolve(process.cwd(), "public/data/sites.json"),
  "utf8",
);
...
const OPPORTUNITY_FIXTURE_VALID_FROM = JSON.parse(
  readFileSync(
...
async function accountBrowserStorageSnapshot(page: Page) {
  return page.evaluate(
    (keys) => ({
      ...
      sessionAccountValues: keys.map((key) =>
        window.sessionStorage.getItem(key),
      ),
      localPreference: window.localStorage.getItem(
        "castingcompass.respect-water.v1",
      ),
      sessionPreference: window.sessionStorage.getItem(
        "castingcompass.respect-water.v1",
      ),
      localUnrelated: window.localStorage.getItem("unrelated.site.preference"),
      sessionUnrelated: window.sessionStorage.getItem(
        "unrelated.site.preference",
      ),
    }),
    ACCOUNT_BROWSER_STORAGE_KEYS,
  );
...
function expectAccountBrowserStorageCleared(
  snapshot: Awaited<ReturnType<typeof accountBrowserStorageSnapshot>>,
) {
  expect(snapshot.localAccountValues.every((value) => value === null)).toBe(
    true,
  );
  expect(snapshot.sessionAccountValues.every((value) => value === null)).toBe(
    true,
  );
...
async function preparePastTripForSubmission(page: Page) {
  ...
  await expect(modal.locator(".site-combobox-status")).toHaveText(
    /^Selected: .+$/,
  );
  ...
  await modal
    .getByLabel("Did the score influence this trip?")
    .selectOption("no");
  await modal
    .getByRole("button", { name: "Continue to gear + result" })
    .click();
  await expect(
    modal.getByRole("button", { name: "Record no-fish trip" }),
  ).toBeVisible();
  for (const checkbox of await modal.locator(".consent-field input").all())
...
function pastTripReceipt(route: Route) {
  const tripId = route
    .request()
    .postData()
    ?.match(/trip_[a-f0-9-]{36}/)?.[0];
  if (!tripId)
    throw new Error(
      "Past-trip request did not include a client trip identity.",
    );
  return {
    trip: {
      id: tripId,
...
async function prepareAccountDeletion(page: Page) {
  ...
  await expect(
    modal.getByRole("heading", { name: "Your fishing profile." }),
  ).toBeVisible();
  const deletion = modal.locator(
    ".profile-privacy-section .account-delete-details",
  );
...
async function prepareTripDeletion(page: Page) {
  ...
  const trip = modal
    .locator(".profile-trip")
    .filter({ hasText: "Limantour Beach" });
...
async function prepareTripEdit(page: Page) {
  ...
  await expect(
    editor.getByRole("heading", { name: "Edit trip log" }),
  ).toBeVisible();
  await editor
    .getByLabel("Notes", { exact: true })
    .fill("Draft retained during recovery testing");
...
async function prepareSavedSiteMutation(page: Page) {
  ...
  await expect(
    detail.getByRole("heading", { name: "Limantour Beach" }),
  ).toBeVisible();
...
async function expectSelectorInsideViewport(page: Page, selector: string) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    await expect(async () => {
      ...
      expect(
        box!.x,
        `${selector} starts inside the viewport`,
      ).toBeGreaterThanOrEqual(-1);
      expect(
        box!.x + box!.width,
        `${selector} ends inside the viewport`,
...
async function ensureInteractiveMap(page: Page) {
  ...
  await expect(async () => {
    ...
    if (await loadMap.isVisible())
      await loadMap.click({ timeout: 1_000 }).catch(() => undefined);
...
test.beforeEach(async ({ page }, testInfo) => {
  ...
  await page.route("**/data/sites.json", (route) =>
    route.fulfill({
      status: 200,
  ...
  await page.route("**/data/structure-depth.json", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
  ...
  const profileRecoveryTest = testTitle.includes(
    "failed profile load stays unknown",
  );
  const tripRecoveryTest =
    testTitle.includes("trip submissions pause while offline") ||
    testTitle.includes("slow trip save stays pending") ||
    testTitle.includes("failed trip save remains ambiguous");
  const accountDeletionRecoveryTest =
    testTitle.includes("account deletion pauses while offline") ||
  ...
  const tripDeletionRecoveryTest =
    testTitle.includes("trip deletion pauses while offline") ||
    testTitle.includes("slow trip deletion stays unconfirmed") ||
    testTitle.includes("failed trip deletion stays ambiguous");
  const tripEditRecoveryTest =
    testTitle.includes("trip edit pauses while offline") ||
  ...
  const gearMutationRecoveryTest =
    testTitle.includes("gear changes pause while offline") ||
  ...
  const signOutRecoveryTest =
    testTitle.includes("sign-out pauses while offline") ||
    testTitle.includes("exact sign-out receipt clears only") ||
    testTitle.includes(
      "sign-out warns when browser storage cleanup is blocked",
    ) ||
  ...
  const savedSiteRecoveryTest =
    testTitle.includes("saved-location changes pause while offline") ||
  ...
  const structureDepthEvidenceTest =
    testTitle.includes("source-bound Santa Barbara chart context") ||
  ...
  if (waterQualityAdvisoryTest) {
    ...
    const assessment = (overrides: Record<string, unknown>) => ({
      ...
      detail:
        "Neutral context only. This does not mean the water or seafood is safe and does not improve the fishing score.",
      sourceId: "sfpuc",
      stationIds: ["4612"],
    ...
    await page.route("**/data/water-quality.json", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
          ...
          sites: {
            "baker-beach": assessment({
              ...
              detail:
                "This active agency status suppresses the site from CastingCompass recommendations.",
            ...
            "gaviota-state-park-beach": assessment({
              ...
              officialLabel: "Official water-contact posting",
              detail:
                "A current county-submitted action in the official State Board table suppresses this site from recommendations.",
            ...
            "pacifica-state-beach": assessment({
              ...
              detail:
                "The current County Health posting list names an exact reviewed station for this site, so the recommendation is suppressed.",
            ...
            "keller-beach": assessment({
              ...
              officialLabel: "Official water-contact posting",
              detail:
                "A current district-submitted action in the official State Board table suppresses this site from recommendations.",
    ...
    route.fulfill({
      ...
      body: JSON.stringify({
        user: forecastRecoveryTest
          ? {
              id: "user_forecast_recovery",
              email: "forecasttest@example.com",
              ageEligible: true,
              legalAccepted: true,
            }
          : profileRecoveryTest
            ? {
                id: "user_profile_recovery",
            ...
            : tripRecoveryTest
              ...
              : accountDeletionRecoveryTest
                ...
                : tripDeletionRecoveryTest
                  ? {
                      id: "user_trip_deletion",
                      email: "tripdeletiontest@example.com",
                      ageEligible: true,
                      legalAccepted: true,
                    }
                  : tripEditRecoveryTest
                    ? {
                        id: "user_trip_edit",
                        email: "tripedittest@example.com",
                        ageEligible: true,
                        legalAccepted: true,
                      }
                    : gearMutationRecoveryTest
                      ? {
                          id: "user_gear_recovery",
                          email: "geartest@example.com",
                          ageEligible: true,
                          legalAccepted: true,
                        }
                      : signOutRecoveryTest
                        ? {
                            id: "user_signout_recovery",
  ...
  if (tripDeletionRecoveryTest) {
    await page.route("**/api/profile/reviews/retry", (route) =>
      route.fulfill({
        status: 202,
        contentType: "application/json",
  ...
  if (signOutRecoveryTest) {
    ...
    await page.route("**/api/auth/turnstile-config", (route) =>
      route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          turnstile: { enabled: false, available: false, siteKey: null },
        }),
      }),
    );
  }
  await page.route("**/api/privacy/deletion-status", (route) =>
    route.fulfill({
      status: 404,
      contentType: "application/json",
  ...
  await page.route("**/api/trips/summary", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
  ...
  await page.addInitScript(() => {
    if (
      !new URL(window.location.href).searchParams.has("showRespectReminder")
    ) {
  ...
  if (forecastRecoveryTest) {
    await expect(
      page.locator(".forecast-state-card.unavailable"),
    ).toBeVisible();
  } else {
...
test("forecast failure hides unverified scores until an explicit retry succeeds", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "Start a trip" }),
  ).toBeDisabled();
  await expect(
    page.getByRole("button", { name: "Log a past trip" }),
  ).toBeDisabled();
...
test("primary controls stay inside common phone viewports", async ({
  page,
}) => {
...
async function installLoaderTelemetry(page: Page, telemetryKey: string) {
  await page.addInitScript((key) => {
    const telemetry: LoaderTelemetry = {
      frames: [],
      animationSettings: null,
      animationKeyframes: null,
    };
    (window as unknown as Record<string, unknown>)[key] = telemetry;

    const waitForLoader = () => {
      const loader = document.querySelector<HTMLElement>(".cc-topo-loader");
      if (!loader) {
      ...
      const sample = () => {
        ...
        const map = loader.querySelector<HTMLElement>(
          ".cc-topographic-map-loader",
        );
        const spreads = [
          ...loader.querySelectorAll<SVGPathElement>(
            ".cc-topo-ridge-spreads path",
          ),
        ];
        const revealing = document
          .querySelector(".cc-landing")
          ?.classList.contains("cc-intro-revealing");
        if (revealing && !telemetry.animationSettings && spreads.length > 0) {
          telemetry.animationSettings = spreads.map((path) => {
          ...
          const animation = spreads[0].getAnimations()[0];
          telemetry.animationKeyframes =
        ...
        telemetry.frames.push({
          ...
          mapTransform: map ? getComputedStyle(map).transform : "missing",
          spreadScales: spreads.map((path) => {
            const transform = getComputedStyle(path).transform;
            if (transform === "none") return 1;
            return new DOMMatrixReadOnly(transform).a;
          }),
        });
        if (loader.isConnected) requestAnimationFrame(sample);
      };
...
async function readLoaderTelemetry(page: Page, telemetryKey: string) {
  return page.evaluate(
    (key) =>
      (window as unknown as Record<string, LoaderTelemetry>)[key] ?? {
        frames: [],
        animationSettings: null,
      },
    telemetryKey,
...
test("marketing loader stays fixed and clears every traced peak on one fast-slow-fast timeline", async ({
  page,
}) => {
  test.setTimeout(20_000);
  await page.emulateMedia({ reducedMotion: "no-preference" });
  await installLoaderTelemetry(page, "__ccMobileLoaderTelemetry");
  await page.goto("/");

  const maskPaths = page.locator(".cc-topo-ridge-spreads path");
  await expect(maskPaths).toHaveCount(4);
  const maskPaint = await maskPaths.evaluateAll((paths) =>
    paths.map((path) => {
      const style = getComputedStyle(path);
      return {
        fill: style.fill,
  ...
  expect(maskPaint.every(({ fill }) => fill === "rgb(0, 0, 0)")).toBe(true);
  expect(maskPaint.every(({ stroke }) => stroke === "none")).toBe(true);
  expect(
    maskPaint.every(({ transformBox }) => transformBox === "fill-box"),
  ).toBe(true);
  ...
  const telemetry = await readLoaderTelemetry(
    page,
    "__ccMobileLoaderTelemetry",
  );

  expect(telemetry.frames.length).toBeGreaterThan(10);
  expect(
    Math.max(...telemetry.frames.map(({ x }) => x)) -
      Math.min(...telemetry.frames.map(({ x }) => x)),
  ).toBeLessThanOrEqual(1);
  expect(
    Math.max(...telemetry.frames.map(({ y }) => y)) -
      Math.min(...telemetry.frames.map(({ y }) => y)),
  ).toBeLessThanOrEqual(1);
  expect(
    Math.max(...telemetry.frames.map(({ clientWidth }) => clientWidth)) -
      Math.min(...telemetry.frames.map(({ clientWidth }) => clientWidth)),
  ).toBeLessThanOrEqual(1);
  expect(
    telemetry.frames.every(({ mapTransform }) => mapTransform === "none"),
  ).toBe(true);

  const animationSettings = telemetry.animationSettings ?? [];
  expect(animationSettings).toHaveLength(4);
  expect(animationSettings.every(({ delay }) => delay === "0s")).toBe(true);
  expect(animationSettings.every(({ duration }) => duration === "2s")).toBe(
    true,
  );
  expect(animationSettings.every(({ timing }) => timing === "linear")).toBe(
    true,
  );
  expect(telemetry.animationKeyframes).toEqual([
    { offset: 0, transform: "scale(1)" },
    { offset: 0.2, transform: "scale(11)" },
    { offset: 0.75, transform: "scale(21)" },
    { offset: 1, transform: "scale(34)" },
  ]);

  const revealFrames = telemetry.frames.filter(
    ({ spreadScales }) => spreadScales.length === 4 && spreadScales[0] > 1.01,
  );
  expect(revealFrames.length).toBeGreaterThan(4);
  for (const { spreadScales } of revealFrames) {
    expect(
      Math.max(...spreadScales) - Math.min(...spreadScales),
    ).toBeLessThanOrEqual(0.05);
  }
  const revealProgress = revealFrames.map(
    ({ spreadScales }) => spreadScales[0],
  );
  expect(revealProgress.at(-1)).toBeGreaterThan(revealProgress[0]);
  for (let index = 1; index < revealProgress.length; index += 1) {
    expect(revealProgress[index]).toBeGreaterThanOrEqual(
      revealProgress[index - 1],
    );

    const elapsed = revealFrames[index].time - revealFrames[index - 1].time;
    const scaleDelta = revealProgress[index] - revealProgress[index - 1];
    // WebKit may expose an interpolated SVG transform one paint behind the
    // requestAnimationFrame timestamp. A single repeated sample is harmless;
    // a repeated sample across more than two 60 Hz frames is the visible
    // pause/stickiness this regression is intended to prevent.
    if (elapsed > 34) {
      expect(scaleDelta).toBeGreaterThan(0.05);
    }
  }

  const firstRevealFrame = revealFrames[0];
  const lastRevealFrame = revealFrames.at(-1)!;
  const revealDuration = lastRevealFrame.time - firstRevealFrame.time;
  const revealDistance =
    lastRevealFrame.spreadScales[0] - firstRevealFrame.spreadScales[0];
  // The declared CSS timing plus the monotonic/no-plateau assertions above are
  // more stable cross-engine evidence than correlating WebKit's painted SVG
  // matrix with requestAnimationFrame timestamps, which can be one or more
  // frames out of phase under parallel test load.
  expect(revealDuration).toBeGreaterThan(1600);
  expect(revealDuration).toBeLessThan(2300);
  expect(revealDistance).toBeGreaterThan(20);
});

test("marketing loader remains viewport-locked at desktop width", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await installLoaderTelemetry(page, "__ccDesktopLoaderTelemetry");
  await page.goto("/");
  ...
  const telemetry = await readLoaderTelemetry(
    page,
    "__ccDesktopLoaderTelemetry",
  );
  const visibleFrames = telemetry.frames.filter(
    ({ width, height }) => width > 0 && height > 0,
  );
  expect(visibleFrames.length).toBeGreaterThan(10);
  for (const frame of visibleFrames) {
    expect(frame.x).toBeCloseTo(0, 1);
    expect(frame.y).toBeCloseTo(0, 1);
    expect(frame.width).toBeCloseTo(frame.viewportWidth, 1);
    expect(frame.height).toBeCloseTo(frame.viewportHeight, 1);
  }
  expect(
    Math.max(...visibleFrames.map(({ clientWidth }) => clientWidth)) -
      Math.min(...visibleFrames.map(({ clientWidth }) => clientWidth)),
  ).toBeLessThanOrEqual(1);
  await expect(page.locator(".cc-topo-loader")).toHaveCount(0);
});

test("marketing loader resolves immediately without decorative motion when reduced motion is requested", async ({
  page,
}) => {
...
test("marketing homepage resolves its loading artwork into a responsive planning hero", async ({
  page,
}) => {
  ...
  await expect(
    page
      .locator(".cc-opening")
      .getByRole("link", { name: "Find best spot near you" }),
  ...
  await expect(page.locator("body")).toContainText("Download on TestFlight");
  await expect(page.locator("body")).not.toContainText(
    "Trusted by 25,000+ anglers",
  );
  await expect(page.locator("body")).not.toContainText(
    "Sample community preview",
  );
  await expect(page.locator("body")).not.toContainText("Field map 01");
  ...
  expect(heroFrame.area).toBeLessThan(heroFrame.viewportArea * 0.7);
  expect(heroFrame.borderRadius).not.toBe("0px");

  const mobileHeroSpacing = await page.evaluate(() => {
    const media = document
      .querySelector<HTMLElement>(".cc-hero-topo-panel")!
      .getBoundingClientRect();
    const actions = document
      .querySelector<HTMLElement>(".cc-hero-actions")!
      .getBoundingClientRect();
    const utility = document
      .querySelector<HTMLElement>(".cc-utility-copy")!
      .getBoundingClientRect();
    return {
      mediaTop: media.top,
      contentBottom: Math.max(actions.bottom, utility.bottom),
    };
  });
  expect(mobileHeroSpacing.contentBottom).toBeLessThanOrEqual(
...
test("forecast and presentation choices announce their selected state", async ({
  page,
}) => {
...
test("target selection is visible, fast-swappable, URL-bound, and persisted", async ({
  page,
}) => {
  ...
  for (const target of targets) {
    ...
    await expectSelectorInsideViewport(
      page,
      `.species-switcher button:has-text("${target}")`,
    );
  }

  const surfperch = page.getByRole("button", {
    name: "Surfperch",
    exact: true,
  });
  await surfperch.click();
  await expect(surfperch).toHaveAttribute("aria-pressed", "true");
  await expect(page.locator(".forecast-intro .eyebrow").first()).toContainText(
    "Surfperch planning",
  );
  ...
  await page.reload();
  await expect(
    page.getByRole("button", { name: "Surfperch", exact: true }),
...
test("modal focus is trapped and restored through a nested account surface", async ({
  page,
}, testInfo) => {
  ...
  await page.evaluate(() =>
    document.querySelector<HTMLElement>(".location-button")?.focus(),
  );
  await expect(accountDialog).toBeFocused();
...
test("the required water reminder traps focus and cannot be dismissed with Escape", async ({
  page,
}) => {
  await page.evaluate(() => {
  ...
  await expect(reminder).toBeFocused();
  await reminder
    .getByRole("button", { name: /continue to castingcompass/i })
    .click();
  await expect(reminder).toHaveCount(0);
});

test("the complete forecast reflows without horizontal document scrolling at 320px", async ({
  page,
}) => {
...
test("official water-quality status suppresses recommendations and exposes the Santa Barbara action source", async ({
  page,
}) => {
  ...
  await expect(
    page.locator(".site-card").filter({ hasText: "Baker Beach" }),
  ).toHaveCount(0);
  await expect(
    page.locator(".site-card").filter({ hasText: "Gaviota State Park Beach" }),
  ).toHaveCount(0);
  await expect(
    page.locator(".site-card").filter({ hasText: "Pacifica State Beach" }),
  ).toHaveCount(0);
  await expect(
    page.locator(".site-card").filter({ hasText: "Bolinas Beach" }),
  ).toHaveCount(0);
  await expect(
    page.locator(".site-card").filter({ hasText: "Keller Beach" }),
  ).toHaveCount(0);
  await expect(
    page
      .locator(".site-card")
      .filter({ hasText: "Crown Memorial State Beach" }),
  ).toHaveCount(0);
  ...
  await expect(actionAdvisory).toContainText(
    "Agency action start date: 2026-06-15",
  );
  await expect(actionAdvisory).toContainText("No end date is reported");
  await expect(
    actionAdvisory.getByRole("link", { name: /official agency status/i }),
  ).toHaveAttribute(
    "href",
    "https://beachwatch.waterboards.ca.gov/public/advisory.php",
  );
...
test("official water-quality status exposes the San Mateo sample source", async ({
  page,
}) => {
  ...
  await expect(countyAdvisory).toContainText(
    "Official water-contact warning or closure",
  );
  await expect(countyAdvisory).toContainText("Agency sample date: 2026-07-13");
  await expect(countyAdvisory).toContainText(
    "does not improve this fishing score",
  );
  await expect(
    countyAdvisory.getByRole("link", { name: /official agency status/i }),
  ).toHaveAttribute("href", "https://www.smchealth.org/node/1201");
});

test("official water-quality status exposes the Marin action source", async ({
  page,
}) => {
  ...
  await expect(marinAdvisory).toContainText(
    "Agency action start date: 2026-07-15",
  );
  await expect(marinAdvisory).toContainText(
    "does not improve this fishing score",
  );
  await expect(
    marinAdvisory.getByRole("link", { name: /official agency status/i }),
  ).toHaveAttribute(
    "href",
    "https://beachwatch.waterboards.ca.gov/public/advisory.php",
  );
...
test("official water-quality status exposes the East Bay Parks action source", async ({
  page,
}) => {
  ...
  await expect(eastBayParksAdvisory).toContainText(
    "Official water-contact posting",
  );
  await expect(eastBayParksAdvisory).toContainText(
    "Agency action start date: 2026-05-05",
  );
  await expect(eastBayParksAdvisory).toContainText(
    "does not improve this fishing score",
  );
...
test("official water-quality status keeps neutral status explicit", async ({
  page,
}) => {
  ...
  await expect(
    advisory.getByRole("link", { name: /official agency status/i }),
  ).toHaveAttribute(
    "href",
...
test("source-bound Santa Barbara chart context stays truthful and mobile-safe", async ({
  page,
}) => {
  ...
  await expect(evidence).toContainText(
    "2.4–5.4 m across 3 deduplicated records within 1,000 m",
  );
  ...
  await expect(evidence).toContainText(
    "not for navigation, wading, or access decisions",
  );
  await expect(
    evidence.getByRole("link", { name: /NOAA source notes/i }),
  ).toHaveAttribute("href", "https://nauticalcharts.noaa.gov/learn/encdirect/");
  await expectSelectorInsideViewport(page, ".structure-depth-evidence");
});

test("source-bound San Francisco chart context preserves partial source-date precision", async ({
  page,
}) => {
...
test("source-bound San Francisco chart context keeps a missing sector band explicitly partial", async ({
  page,
}) => {
  ...
  await expect(evidence).toBeVisible();
  await expect(evidence).toContainText(
    "No reviewed NOAA depth-area band intersected this configured sector",
  );
  await expect(evidence).toContainText(
    "4.9–12 m across 7 deduplicated records within 1,000 m",
  );
  await expect(evidence).toContainText("Charted wreck");
  await expect(evidence).toContainText(
    "the gap is not proof of shallow water, safe access, or castability",
  );
  await expectSelectorInsideViewport(page, ".structure-depth-evidence");
});

test("source-bound San Mateo Coast coverage preserves the closed-site recommendation boundary", async ({
  page,
}) => {
  await page.goto("/forecast?site=pacifica-municipal-pier");
  const closure = page
    .locator(".closure-notice")
    .filter({ hasText: "temporarily closed access point" });

  await expect(closure).toContainText(
    "1 temporarily closed access point is excluded from ranking",
  );
  await expect(
    closure.getByRole("link", { name: /official status/i }),
  ).toHaveAttribute(
    "href",
    "https://www.cityofpacifica.org/departments/public-works/field-services/pacifica-pier",
  );
  await expect(
    page.locator(".site-card").filter({ hasText: "Pacifica Municipal Pier" }),
  ).toHaveCount(0);
...
test("source-bound San Mateo Coast chart context preserves Half Moon Bay date precision", async ({
  page,
}) => {
  ...
  await expect(evidence).toContainText(
    "2.4–9.1 m across 9 deduplicated records within 1,000 m",
  );
...
test("source-bound Marin Coast chart context keeps a missing sector band explicitly partial", async ({
  page,
}) => {
  await page.goto("/forecast?site=bolinas-beach");
  const evidence = page.locator(".structure-depth-evidence");
  ...
  await expect(evidence).toContainText(
    "0.3–4.2 m across 6 deduplicated records within 1,000 m",
  );
...
test("source-bound Marin Coast chart context preserves Point Reyes date precision", async ({
  page,
}) => {
  ...
  await expect(evidence).toContainText(
    "0.4–9.6 m across 14 deduplicated records within 1,000 m",
  );
  await expect(evidence).toContainText("Charted seabed description");
  await expect(evidence).toContainText("some dates have year/month precision");
  await expect(evidence).toContainText("some records have no source date");
...
test("source-bound North and East Bay chart context keeps a missing sector band explicitly partial", async ({
  page,
}) => {
  ...
  await expect(evidence).toContainText(
    "0.6–2.4 m across 6 deduplicated records within 1,000 m",
  );
  await expect(evidence).toContainText("Charted shoreline construction");
  await expect(evidence).toContainText(
    "the gap is not proof of shallow water, safe access, or castability",
...
test("source-bound North and East Bay chart context preserves Berkeley date precision", async ({
  page,
}) => {
  ...
  await expect(evidence).toContainText("0–1.8 m");
  await expect(evidence).toContainText(
    "0.3–1.8 m across 8 deduplicated records within 1,000 m",
  );
  await expect(evidence).toContainText("Charted obstruction");
  await expect(evidence).toContainText("some dates have year/month precision");
  await expect(evidence).toContainText("some records have no source date");
...
test("source-bound Oakland through South Bay chart context preserves Port View depth and date limits", async ({
  page,
}) => {
  ...
  await expect(evidence).toContainText(
    "2.1–13.1 m across 12 deduplicated records within 1,000 m",
  );
...
test("source-bound Oakland through South Bay chart context keeps Coyote Point display-only", async ({
  page,
}) => {
  ...
  await expect(evidence).toContainText("0–1.8 m");
  await expect(evidence).toContainText(
    "0.6–2.1 m across 12 deduplicated records within 1,000 m",
  );
...
test("safe-area contract keeps fixed controls inside simulated insets", async ({
  page,
}) => {
  ...
  const modal = await page.locator(".account-modal-layer").evaluate((layer) => {
    const box = layer
      .querySelector<HTMLElement>(".account-modal")!
      .getBoundingClientRect();
  ...
  expect(modal.bottom).toBeLessThanOrEqual(
    modal.viewportHeight - insets.bottom,
  );
});
...
test("map tile cleanup suppresses only the known MapLibre abort rejection", async ({
  page,
}) => {
  ...
  const dispatchResults = await page.evaluate(() => {
    ...
    return {
      ...
      mapLibreFailurePropagated: dispatchRejection({
        ...
        stack:
          "TypeError: Raster source failed\n    at q.abortTile (https://castingcompass.com/assets/maplibre-gl-2DjS9JS6.js:1:2)",
      }),
    };
  });
...
test("opening multiple location reports does not leak map tile aborts", async ({
  page,
}) => {
  ...
  expect(
    pageErrors.filter((message) =>
      /signal is aborted without reason|AbortError/i.test(message),
    ),
  ).toEqual([]);
});

test("the 404 recovery page stays truthful and usable on mobile", async ({
  page,
}) => {
  ...
  const geometry = await page.evaluate(() => ({
    ...
    card: (() => {
      const box = document
        .querySelector<HTMLElement>(".not-found-card")!
        .getBoundingClientRect();
      return {
        left: box.left,
        right: box.right,
  ...
  expect(geometry.card.right).toBeLessThanOrEqual(
    geometry.card.viewportWidth + 1,
  );
});
...
test.describe("route render recovery", () => {
  ...
  test("a failed lazy route dependency reaches the safe mobile recovery boundary", async ({
    page,
  }) => {
    const map = page.locator(".map-wrap");
    ...
    const loadMap = page.getByRole("button", { name: /open interactive map/i });
    if (await loadMap.isVisible())
      await loadMap.click({ timeout: 2_000 }).catch(() => undefined);
    ...
    await expect(
      page.getByRole("link", { name: "Return to the forecast" }),
    ).toHaveAttribute("href", "/forecast");
    await expect(alert).not.toContainText(/TypeError|ContourMap|digest|stack/i);

    const geometry = await page.evaluate(() => {
      const box = document
        .querySelector<HTMLElement>(".route-state-card")!
        .getBoundingClientRect();
      return {
        overflow: document.documentElement.scrollWidth - window.innerWidth,
        left: box.left,
...
test.describe("profile recovery", () => {
  ...
  test("a failed profile load stays unknown until an explicit retry succeeds", async ({
    page,
  }) => {
    ...
    await expect(loadError).toContainText(
      "CastingCompass is not treating the account as empty.",
    );
    expect(
      await page.locator(".profile-summary strong").allTextContents(),
    ).toEqual(["—", "—"]);
    await expect(
      page.getByText("No saved locations yet.", { exact: false }),
    ).toHaveCount(0);
    await expect(
      page.getByText(
        "Saved locations are unavailable. Retry the profile above.",
      ),
    ).toBeVisible();
    ...
    await expect(
      page.getByText("Limantour Beach", { exact: true }),
    ).toBeVisible();
...
test.describe("account entry recovery", () => {
  ...
  test("sign-in pauses while offline and never submits automatically after reconnection", async ({
    page,
    context,
  }) => {
    let loginRequests = 0;
    ...
    await expect(
      modal.getByRole("button", { name: "Reconnect to continue" }),
    ).toBeDisabled();
    await expect(
      modal
        .getByRole("alert")
        .filter({ hasText: "Account requests are paused" }),
    ).toBeVisible();
    expect(loginRequests).toBe(0);

    await context.setOffline(false);
    const retryVerification = modal.getByRole("button", {
      name: "Retry security verification",
    });
    ...
    await expect(modal.getByRole("status")).toContainText(
      "No account request was submitted automatically",
    );
    await page.waitForTimeout(200);
    expect(loginRequests).toBe(0);
  });

  test("slow sign-in stays unconfirmed until an exact account receipt arrives", async ({
    page,
  }) => {
    let signedIn = false;
    ...
    await modal
      ...
      .click();
    await expect(modal.getByRole("status")).toContainText(
      "No session is confirmed yet",
    );
    await expect(modal.getByRole("status")).toContainText(
      "Do not assume this browser is signed in yet",
      { timeout: 5_500 },
    );
  ...
  test("lost sign-in response requires a read-only session check before retry", async ({
    page,
  }) => {
    ...
    await expect(
      modal
        .getByRole("alert")
        .filter({ hasText: "A session may already exist" }),
    ).toBeVisible();
    await expect(
      modal.getByRole("button", { name: "Account status unresolved" }),
    ).toBeDisabled();

    await modal.getByRole("button", { name: "Check account status" }).click();
    await expect(
      modal.getByRole("button", { name: "Checking…" }),
    ).toBeVisible();
    await expect(
      modal.getByRole("button", { name: "Checking…" }),
    ).toBeDisabled();
    ...
    await expect(
      modal.getByRole("alert").filter({ hasText: "not signed in" }),
    ).toBeVisible();
    await expect(
      modal
        .locator("form")
  ...
  test("a successful status with an over-broad sign-in receipt stays unresolved", async ({
    page,
  }) => {
    await page.route("**/api/auth/login", (route) =>
      route.fulfill({
  ...
  test("password recovery requires the exact requested challenge receipt", async ({
    page,
  }) => {
    await page.route("**/api/auth/password/request", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
    ...
    await expect(modal.getByRole("alert")).toContainText(
      "no usable challenge receipt",
    );
    await expect(
      modal.getByRole("heading", { name: "Reset your password." }),
    ).toBeVisible();
  });

  test("lost password-reset response directs the user to sign in instead of reusing the code", async ({
    page,
  }) => {
    await page.route("**/api/auth/password/request", (route) =>
      route.fulfill({
    ...
    await modal.getByRole("button", { name: "Email reset code" }).click();
    await expect(
      modal.getByRole("heading", { name: "Enter your reset code." }),
    ).toBeVisible();
...
test.describe("trip network recovery", () => {
  ...
  test("trip submissions pause while offline and never replay automatically", async ({
    page,
    context,
  }) => {
    ...
    await page.route("**/api/trips/report", (route) => {
      ...
      return route.fulfill({
        status: 201,
        contentType: "application/json",
        body: JSON.stringify({ tripId: "unexpected" }),
      });
    });
    ...
    await context.setOffline(true);
    await expect(modal.getByRole("alert")).toContainText(
      "trip submissions are paused",
    );
    await expect(
      modal.getByRole("button", { name: "Reconnect to save report" }),
    ).toBeDisabled();
    ...
    await expect(
      modal
        .getByRole("status")
        .filter({ hasText: "Nothing was resubmitted automatically" }),
    ).toBeVisible();
    await expect(
      modal.getByRole("button", { name: "Record no-fish trip" }),
    ).toBeEnabled();
  ...
  test("a slow trip save stays pending until authoritative confirmation", async ({
    page,
  }) => {
    let reportAttempts = 0;
    ...
    await expect(status).toContainText("no report is confirmed yet", {
      timeout: 5_500,
    });
    ...
    await expect(status).toContainText(
      "No-fish trip recorded and pending review",
      { timeout: 8_000 },
    );
    await expect(
      modal.getByRole("button", { name: "Report saved" }),
    ).toBeDisabled();
    expect(
      await page.evaluate(() =>
        window.localStorage.getItem("castingcompass.trip-draft.v1.past"),
      ),
    ).toBeNull();
  });

  test("a failed trip save remains ambiguous and keeps its draft", async ({
    page,
  }) => {
    let reportAttempts = 0;
    ...
    await expect(alert).toContainText(
      "retrying here is safe and cannot create a duplicate",
    );
    await expect(
      modal.getByRole("button", { name: "Retry safely" }),
    ).toBeEnabled();
    await expect(
      modal.locator("fieldset.trip-write-fields input").first(),
    ).toBeDisabled();
    expect(reportAttempts).toBe(1);
    expect(
      await page.evaluate(() =>
    ...
    expect(
      await page.evaluate(() =>
        window.localStorage.getItem("castingcompass.trip-request.v1.past"),
      ),
    ).not.toBeNull();
    expect(
      await page.evaluate(() =>
        window.localStorage.getItem("castingcompass.trip-pending.v1.past"),
      ),
    ).not.toBeNull();
...
test.describe("account deletion recovery", () => {
  ...
  test("account deletion pauses while offline and never submits automatically", async ({
    page,
    context,
  }) => {
    ...
    await expect(deletion.getByRole("alert")).toContainText(
      "Account deletion has not been submitted",
    );
    await expect(
      deletion.getByRole("button", { name: "Reconnect to delete account" }),
    ).toBeDisabled();

    await context.setOffline(false);
    await expect(deletion.getByRole("status")).toContainText(
      "No deletion request was submitted automatically",
    );
    await expect(
  ...
  test("confirmed account deletion clears only account-bound browser recovery state", async ({
    page,
  }) => {
    await page.route("**/api/profile", (route) => {
      if (route.request().method() === "GET") {
    ...
    await deletion
      ...
      .click();
    await expect(
      modal.getByRole("heading", { name: "Account access removed." }),
    ).toBeVisible();
    expectAccountBrowserStorageCleared(
      await accountBrowserStorageSnapshot(page),
    );
  });

  test("a slow account deletion stays unconfirmed until the receipt arrives", async ({
    page,
  }) => {
    ...
    await expect(status).toContainText(
      "account removal has not been confirmed yet",
      { timeout: 5_500 },
    );
    await expect(status.locator("i")).toBeVisible();
    await expect(
      deletion.getByRole("button", { name: "Deleting…" }),
    ).toBeDisabled();
    await expect(
      modal.getByRole("heading", { name: "Account access removed." }),
  ...
  test("a failed account deletion stays ambiguous and warns against resubmission", async ({
    page,
  }) => {
    let deletionAttempts = 0;
    await page.route("**/api/profile", (route) => {
    ...
    await expect(
      deletion.getByRole("button", {
        name: "Verify deletion status before retrying",
      }),
    ).toBeDisabled();
...
test.describe("trip deletion recovery", () => {
  ...
  test("trip deletion pauses while offline and never submits automatically", async ({
    page,
    context,
  }) => {
    let deletionAttempts = 0;
    ...
    const { modal, trip } = await prepareTripDeletion(page);
    await context.setOffline(true);
    await expect(modal.locator(".trip-deletion-network-status")).toContainText(
      "Trip deletion is paused",
    );
    await expect(
      trip.getByRole("button", { name: "Reconnect to remove" }),
    ).toBeDisabled();

    await context.setOffline(false);
    await expect(modal.locator(".trip-deletion-network-status")).toContainText(
      "No trip edit or deletion was submitted automatically",
    );
  ...
  test("a slow trip deletion stays unconfirmed until the receipt arrives", async ({
    page,
  }) => {
    ...
    await expect(status).toContainText(
      "trip deletion has not been confirmed yet",
      { timeout: 5_500 },
    );
    await expect(status.locator("i")).toBeVisible();
    await expect(
      trip.getByRole("button", { name: "Removing…" }),
    ).toBeDisabled();
    await expect(
      modal.getByRole("heading", { name: "Trip log removed." }),
    ).toBeVisible({ timeout: 8_000 });
  });

  test("a failed trip deletion stays ambiguous and blocks resubmission", async ({
    page,
  }) => {
    let deletionAttempts = 0;
    await page.route("**/api/profile/trips/*", (route) => {
...
test.describe("trip edit recovery", () => {
  ...
  test("trip edit pauses while offline and never submits automatically", async ({
    page,
    context,
  }) => {
    let editAttempts = 0;
    ...
    await expect(editor.getByRole("alert")).toContainText(
      "draft remains on this device",
    );
    await expect(
      editor.getByRole("button", { name: "Reconnect to save changes" }),
    ).toBeDisabled();

    await context.setOffline(false);
    await expect(editor.getByRole("status")).toContainText(
      "No trip edit was submitted automatically",
    );
    await expect(
      editor.getByRole("button", { name: "Save trip changes" }),
    ).toBeEnabled();
    await page.waitForTimeout(100);
    expect(editAttempts).toBe(0);
  });

  test("a slow trip edit stays unconfirmed until the matching receipt arrives", async ({
    page,
  }) => {
    await page.route("**/api/profile/trips/*", async (route) => {
      await new Promise((resolve) => setTimeout(resolve, 6_000));
    ...
    await expect(status).toContainText(
      "trip update has not been confirmed yet",
      { timeout: 5_500 },
    );
    await expect(status.locator("i")).toBeVisible();
    await expect(
    ...
    await expect(modal.getByRole("status")).toContainText(
      "Saved. Because this completed report was edited",
    );
    expect(
      await page.evaluate(() =>
        window.localStorage.getItem(
          "castingcompass.profile-trip-draft.v1.trip_pending_edit",
        ),
      ),
    ).toBeNull();
  });

  test("a failed trip edit stays ambiguous, retains its draft, and blocks conflicting writes", async ({
    page,
  }) => {
    let editAttempts = 0;
    await page.route("**/api/profile/trips/*", (route) => {
    ...
    await expect(alert).toContainText(
      "These trip changes may already be saved",
    );
    await expect(alert).toContainText("Do not submit again");
    await expect(
      editor.getByRole("button", { name: "Verify saved trip before retrying" }),
    ).toBeDisabled();
    expect(
      await page.evaluate(() =>
        window.localStorage.getItem(
    ...
    await expect(
      trip.getByRole("button", {
        name: "Verify saved trip before editing again",
      }),
    ).toBeDisabled();
    await expect(
      trip.getByRole("button", { name: "Trip update unresolved" }),
    ).toBeDisabled();
    expect(editAttempts).toBe(1);
  });

  test("a rejected trip edit remains correctable without losing its draft", async ({
    page,
  }) => {
    let editAttempts = 0;
    await page.route("**/api/profile/trips/*", (route) => {
      ...
      return route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          error: { message: "Check the trip times and try again." },
        }),
      });
    });
    ...
    await editor.getByRole("button", { name: "Save trip changes" }).click();
    await expect(editor.getByRole("alert")).toContainText(
      "Check the trip times and try again",
    );
    await expect(
      editor.getByRole("button", { name: "Save trip changes" }),
    ).toBeEnabled();
...
test.describe("gear mutation recovery", () => {
  ...
  test("gear changes pause while offline and never submit automatically", async ({
    page,
    context,
  }) => {
    ...
    await page.route("**/api/gear-profiles**", (route) => {
      ...
      return route.fulfill({
        ...
        body: JSON.stringify({
          gearProfile: {
            id: "gear_22222222-2222-4222-8222-222222222222",
            name: "Recovery test preset",
            rod: null,
            reel: null,
            baitLure: null,
            rig: null,
    ...
    await context.setOffline(true);
    await expect(gear.getByRole("alert")).toContainText(
      "Gear changes cannot be submitted",
    );
    await expect(
      gear.getByRole("button", { name: "Reconnect to save preset" }),
    ).toBeDisabled();
    await expect(
      gear.getByRole("button", { name: "Reconnect to remove" }),
    ).toBeDisabled();

    await context.setOffline(false);
    await expect(gear.getByRole("status")).toContainText(
      "No gear change was submitted automatically",
    );
    await expect(
      gear.getByRole("button", { name: "Save gear preset" }),
    ).toBeEnabled();
    await expect(gear.getByRole("button", { name: "Remove" })).toBeEnabled();
    expect(mutationAttempts).toBe(0);
  });

  test("slow gear creation stays unconfirmed until the matching resource arrives", async ({
    page,
  }) => {
    ...
    await expect(status).toContainText(
      "new gear preset has not been confirmed yet",
      { timeout: 7_500 },
    );
    await expect(status.locator("i")).toBeVisible();
    await expect(gear.getByRole("button", { name: "Saving…" })).toBeDisabled();
    await expect(gear.getByLabel("Preset name")).toHaveValue("", {
      timeout: 8_000,
    });
  });

  test("failed gear creation stays ambiguous and retains the form", async ({
    page,
  }) => {
    ...
    await expect(gear.getByLabel("Preset name")).toHaveValue(
      "Recovery test preset",
    );
    await expect(
      gear.getByRole("button", { name: "Verify gear status before retrying" }),
    ).toBeDisabled();
    await expect(
      gear.getByRole("button", { name: "Gear change unresolved" }),
    ).toBeDisabled();
    expect(createAttempts).toBe(1);
  });

  test("rejected gear creation remains correctable without losing the form", async ({
    page,
  }) => {
    let createAttempts = 0;
    await page.route("**/api/gear-profiles**", (route) => {
      ...
      return route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          error: { message: "Give this preset a shorter name." },
        }),
      });
    });
    ...
    await gear.getByRole("button", { name: "Save gear preset" }).click();
    await expect(gear.getByRole("alert")).toContainText(
      "Give this preset a shorter name",
    );
    await expect(gear.getByLabel("Preset name")).toHaveValue(
      "Recovery test preset",
    );
  ...
  test("failed gear removal stays ambiguous and blocks every gear write", async ({
    page,
  }) => {
    ...
    await expect(alert).toContainText("Do not submit again");
    await expect(
      gear.getByRole("button", { name: "Verify gear removal before retrying" }),
    ).toBeDisabled();
    await expect(
      gear.getByRole("button", { name: "Verify gear status before retrying" }),
    ).toBeDisabled();
...
test.describe("saved-location mutation recovery", () => {
  ...
  test("saved-location changes pause while offline and never submit automatically", async ({
    page,
    context,
  }) => {
    let mutationAttempts = 0;
    ...
    await expect(
      controls.getByRole("button", {
        name: "Reconnect to remove saved location",
      }),
    ).toBeDisabled();
    await expect(controls.getByRole("alert")).toContainText(
      "No saved-location change was submitted",
    );

    await context.setOffline(false);
    await expect(
      controls.getByRole("button", { name: "Saved location" }),
    ).toBeEnabled();
    await expect(controls.getByRole("status")).toContainText(
      "No saved-location change was submitted automatically",
    );
    await page.waitForTimeout(100);
    expect(mutationAttempts).toBe(0);
  });

  test("slow saved-location removal stays unconfirmed until the exact receipt arrives", async ({
    page,
  }) => {
    ...
    await expect(status).toContainText(
      "saved-location change has not been confirmed yet",
      { timeout: 5_500 },
    );
    await expect(status.locator("i")).toBeVisible();
    await expect(
      controls.getByRole("button", { name: "Removing saved location…" }),
    ).toBeDisabled();
    await expect(
      controls.getByRole("button", { name: "Save location" }),
    ).toBeEnabled({ timeout: 8_000 });
    await expect(status).toContainText("removed and confirmed by the server");
  });

  test("malformed saved-location receipt stays unresolved until a read-only check confirms it", async ({
    page,
    context,
  }) => {
    let mutationAttempts = 0;
    ...
    await expect(
      controls.getByRole("button", {
        name: "Saved-location status unresolved",
      }),
    ).toBeDisabled();
    await expect(
      controls.getByRole("button", { name: "Check saved-location status" }),
    ).toBeEnabled();

    await context.setOffline(true);
    await expect(
      controls.getByRole("button", {
        name: "Reconnect to check saved-location status",
      }),
    ).toBeDisabled();
    await context.setOffline(false);
    await page.route("**/api/saved-sites", (route) =>
    ...
    await controls
      .getByRole("button", { name: "Check saved-location status" })
      .click();
    await expect(
      controls.getByRole("button", { name: "Save location" }),
    ).toBeEnabled();
...
test.describe("sign-out recovery", () => {
  ...
  test("sign-out pauses while offline and never submits automatically", async ({
    page,
    context,
  }) => {
    ...
    await context.setOffline(true);
    await expect(
      controls.getByRole("button", { name: "Reconnect to sign out" }),
    ).toBeDisabled();

    await context.setOffline(false);
    await expect(
      controls.getByRole("button", { name: "Sign out" }),
    ).toBeEnabled();
    await page.waitForTimeout(100);
    expect(signOutAttempts).toBe(0);
  });

  test("an exact sign-out receipt clears only account-bound browser recovery state", async ({
    page,
  }) => {
    await page.route("**/api/auth/logout", (route) =>
      route.fulfill({
  ...
  test("confirmed sign-out warns when browser storage cleanup is blocked", async ({
    page,
  }) => {
    await page.route("**/api/auth/logout", (route) =>
      route.fulfill({
    ...
    await expect(
      modal.getByText(
        "Signed out. This browser blocked removal of locally stored trip recovery data.",
        { exact: false },
      ),
    ).toBeVisible();
    await expect(
      modal.getByText(
        "Clear CastingCompass site data before sharing this device.",
        { exact: false },
      ),
    ).toBeVisible();
  ...
  test("slow sign-out stays unconfirmed until the exact receipt arrives", async ({
    page,
  }) => {
    ...
    await controls.getByRole("button", { name: "Sign out" }).click();
    const status = controls.getByRole("status");
    await expect(status).toContainText("sign-out is not confirmed yet", {
      timeout: 5_500,
    });
    await expect(status.locator("i")).toBeVisible();
    await expect(
      controls.getByRole("button", { name: "Signing out…" }),
    ).toBeDisabled();
    await expect(
      modal.getByRole("heading", { name: "Welcome back." }),
    ).toBeVisible({ timeout: 8_000 });
  ...
  test("malformed sign-out receipt stays unresolved and preserves local account state", async ({
    page,
    context,
  }) => {
    let signOutAttempts = 0;
    ...
    await expect(
      controls.getByRole("button", { name: "Sign-out status unresolved" }),
    ).toBeDisabled();
    await expect(
      controls.getByRole("button", { name: "Check sign-out status" }),
    ).toBeEnabled();
    await expect(modal.getByText("signouttest@example.com")).toBeVisible();
    await context.setOffline(true);
    await expect(
      controls.getByRole("button", {
        name: "Reconnect to check sign-out status",
      }),
    ).toBeDisabled();
    await context.setOffline(false);
    await expect(
  ...
  test("session check confirms sign-out after a dropped mutation response without replay", async ({
    page,
  }) => {
    let signOutAttempts = 0;
    await page.route("**/api/auth/logout", (route) => {
    ...
    await controls
      .getByRole("button", { name: "Check sign-out status" })
      .click();
    await expect(
      modal.getByRole("heading", { name: "Welcome back." }),
    ).toBeVisible();
  ...
  test("session check permits a retry only when the server confirms the session is active", async ({
    page,
  }) => {
    let signOutAttempts = 0;
    await page.route("**/api/auth/logout", (route) => {
    ...
    await expect(
      controls.getByRole("button", { name: "Retry sign out" }),
    ).toBeEnabled();
    expect(signOutAttempts).toBe(1);
  });
});
...
test.describe("mocked Turnstile browser states", () => {
  ...
  test("Turnstile sign-in stays usable at 320px and 360px", async ({
    page,
  }) => {
    await page.route("**/api/auth/turnstile-config", (route) =>
      route.fulfill({
    ...
    for (const width of [320, 360]) {
      ...
      await expect(
        page.locator('[data-mock-turnstile="compact"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-mock-turnstile-retry="never"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-mock-turnstile-feedback="false"]'),
      ).toBeVisible();
      const submit = page.locator(".account-modal form .account-primary");
      await expect(submit).toBeEnabled();
      await expect(
        page.getByText("Security verification complete."),
      ).toBeVisible();
      ...
      for (const element of geometry.elements) {
        expect(
          element.left,
          `${element.selector} starts inside ${width}px`,
        ).toBeGreaterThanOrEqual(-1);
        expect(
          element.right,
          `${element.selector} ends inside ${width}px`,
        ).toBeLessThanOrEqual(geometry.viewportWidth + 1);
        expect(
          element.internalOverflow,
          `${element.selector} has no horizontal overflow`,
        ).toBeLessThanOrEqual(1);
      }
  ...
  test("an open client re-reads runtime config after off-to-on activation", async ({
    page,
  }) => {
  ...
  test("Turnstile script failure pauses sign-in and a retry performs a fresh load", async ({
    page,
  }) => {
    await page.route("**/api/auth/turnstile-config", (route) =>
      route.fulfill({
    ...
    await expect(
      page.locator(".account-modal form .account-primary"),
    ).toBeDisabled();
    await retry.click();
