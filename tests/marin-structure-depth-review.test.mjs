...
import {
  createMarinStructureDepthReviewTemplate,
  evaluateMarinStructureDepthReview,
  ...
  writeMarinStructureDepthReviewTemplate,
} from "../scripts/verify-santa-barbara-structure-depth-review.mjs";

const root = new URL("../", import.meta.url);
const REVIEWED_COMMIT = "d".repeat(40);
const EVALUATED_AT = new Date("2026-07-22T20:00:00.000Z");
const LOCAL_REVIEWER_A = "61111111-1111-4111-8111-111111111111";
const LOCAL_REVIEWER_B = "62222222-2222-4222-8222-222222222222";
const CHART_REVIEWER = "63333333-3333-4333-8333-333333333333";
...
async function loadSources() {
  const [
    ...
    readFile(new URL("field-review/marin-structure-depth-review-policy.json", root)),
    ...
    readFile(new URL("docs/MARIN-STRUCTURE-DEPTH-REVIEW.md", root), "utf8"),
  ]);
  return {
...
function responseId(role, index) {
  const head = role === "local" ? "80000000" : "90000000";
...
function completeEvidence(sources) {
  const evidence = createMarinStructureDepthReviewTemplate({
    ...sources,
...
function evaluate(sources, evidence, evaluatedAt = EVALUATED_AT) {
  return evaluateMarinStructureDepthReview({
    ...sources,
    evidence,
...
test("the blank Marin policy is exact, source-bound, partial-preserving, and region-isolated", async () => {
  const sources = await loadSources();
  const result = validateMarinStructureDepthReview(sources);
  assert.deepEqual(result, {
    schemaVersion: "castingcompass.marin-structure-depth-review/1.0.0",
  ...
  assert.equal(sources.artifact.sites["bolinas-beach"].status, "partial");
  assert.equal(sources.artifact.sites["muir-beach"].status, "partial");
  assert.deepEqual(sources.artifact.sites["bolinas-beach"].depth.chartedBandsMeters, []);
  assert.deepEqual(sources.artifact.sites["muir-beach"].depth.chartedBandsMeters, []);
  assert.match(sources.guide, /Bolinas Beach and Muir Beach remain `partial`/u);
  assert.match(sources.guide, /cannot be\s+used to invent the missing bands or promote either site/u);
});

test("complete disjoint Marin review emits only an aggregate non-authorizing receipt", async () => {
  const sources = await loadSources();
  const receipt = evaluate(sources, completeEvidence(sources));
  assert.equal(receipt.schema_version, "castingcompass.marin-structure-depth-review-receipt/1.0.0");
  assert.equal(receipt.structure_depth_review_accepted, true);
  assert.equal(receipt.passing_site_count, 7);
  assert.equal(receipt.qualifying_local_response_count, 7);
  assert.equal(receipt.qualifying_chart_response_count, 7);
...
test("missing, overlapping, stale, and correction-bearing Marin evidence fails closed", async () => {
  const sources = await loadSources();
  const blankReceipt = evaluate(
    sources,
    createMarinStructureDepthReviewTemplate({ ...sources, reviewedCommit: REVIEWED_COMMIT }),
  );
...
test("Marin digests, identity, unsafe text, duplicate IDs, and status promotion are rejected", async () => {
  const sources = await loadSources();
  ...
  for (const siteId of ["bolinas-beach", "muir-beach"]) {
    const promotedArtifact = structuredClone(sources.artifact);
    promotedArtifact.sites[siteId].status = "charted-context";
    assert.throws(
      () => validateMarinStructureDepthReview({ ...sources, artifact: promotedArtifact }),
      new RegExp(`${siteId} evidence status drifted`, "u"),
    );
  }

  const demotedArtifact = structuredClone(sources.artifact);
  demotedArtifact.sites["limantour-beach"].status = "partial";
  assert.throws(
    () => validateMarinStructureDepthReview({ ...sources, artifact: demotedArtifact }),
    /limantour-beach evidence status drifted/u,
  );
});

test("the Marin guarded writer creates one owner-only file and refuses overwrite", async () => {
  const sources = await loadSources();
  const directory = await mkdtemp(join(tmpdir(), "castingcompass-marin-structure-review-"));
  await chmod(directory, 0o700);
  const outputFile = join(directory, "review.json");
  try {
    const receipt = await writeMarinStructureDepthReviewTemplate({
      root: new URL("../", import.meta.url).pathname.replace(/\/$/u, ""),
      sources,
      reviewedCommit: REVIEWED_COMMIT,
      outputFile,
    });
    assert.equal(receipt.schema_version, "castingcompass.marin-structure-depth-review-template-write-receipt/1.0.0");
    assert.equal(receipt.site_count, 7);
...
test("the Marin CLI has no provider client or process execution path", async () => {
  const source = await readFile(new URL("scripts/verify-marin-structure-depth-review.mjs", root), "utf8");
