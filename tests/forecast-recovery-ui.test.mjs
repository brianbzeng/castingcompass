...
test("the forecast starts empty instead of inventing a snapshot ranking", async () => {
  const app = await source("app/components/OpportunityApp.tsx");

  assert.match(app, /type ForecastDataState = "loading" \| "live" \| "snapshot" \| "unavailable"/u);
  assert.match(
    app,
...
test("a failed core forecast read is distinct from a verified static snapshot", async () => {
  const app = await source("app/components/OpportunityApp.tsx");

  assert.match(app, /async function loadForecastData\(signal: AbortSignal\)/u);
  assert.match(app, /fetch\("\/data\/sites\.json", \{ signal \}\)/u);
  assert.match(app, /fetch\("\/data\/opportunities-browser\.json", \{ signal \}\)/u);
  assert.match(
    app,
  ...
  assert.doesNotMatch(
    app,
    /\.catch\(\(\) => \{[\s\S]{0,180}setDataState\("snapshot"\)/u,
  );
...
test("loading and failure suppress rankings and expose a safe explicit retry", async () => {
  ...
  assert.match(app, /const forecastReady = dataState === "live" \|\| dataState === "snapshot"/u);
  ...
  assert.match(app, /Nothing on this screen is being presented as a current forecast/u);
  assert.match(app, /setForecastLoadAttempt\(\(attempt\) => attempt \+ 1\)/u);
  assert.match(app, /disabled=\{!forecastReady \|\| selectedTarget !== "california-halibut"\}/u);
