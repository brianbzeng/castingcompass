...
async function preparePastTrip(page: Page) {
  const trigger = page.getByRole("button", { name: "Log trip" });
  ...
  await modal
    .getByLabel("Fishing mode for the whole trip")
    .selectOption("shore");
  await modal
...
test("selected and rejected photos expose local preview, type, size, and removable state", async ({
  page,
}) => {
  ...
  await expect(selected).toContainText(
    "Selected only—nothing has uploaded yet.",
  );
  await expect(
    selected.getByRole("img", { name: "Selected verification photo preview" }),
  ).toBeVisible();
  ...
  await expect(
    modal.getByText("Choose a photo", { exact: true }),
  ).toBeVisible();
...
test("pending UI stays indeterminate until an exact stored-photo receipt confirms success", async ({
  page,
}) => {
  ...
  const progress = sending.getByRole("progressbar", {
    name: "Sending verification photo with trip report",
  });
  await expect(progress).toHaveAttribute(
    "aria-valuetext",
    "Sending with the report; byte progress is unavailable",
  );
  await expect(progress).not.toHaveAttribute("aria-valuenow", /.+/);
  await expect(sending.getByRole("button", { name: "Remove" })).toHaveCount(0);

  releaseResponse();
  await expect(modal.locator(".photo-field-confirmed")).toContainText(
    "exact trip receipt confirms a private stored photo",
  );
  await expect(modal.locator(".trip-form-status")).toContainText(
    "verification photo is stored privately with the trip",
  );
});

test("an exact receipt that does not confirm the photo stays ambiguous and retries idempotently", async ({
  page,
}) => {
  let attempts = 0;
  ...
  await modal
    .locator('input[type="file"]')
    .setInputFiles({ name: "retry.png", mimeType: "image/png", buffer: PNG });
  await modal.getByRole("button", { name: "Record no-fish trip" }).click();
  ...
  await expect(ambiguous).toContainText(
    "cancel control is intentionally unavailable",
  );
  await expect(ambiguous.getByRole("button", { name: "Remove" })).toHaveCount(
    0,
  );
...
test("an authoritative rejection exposes remove and explicit whole-report retry", async ({
  page,
}) => {
  let attempts = 0;
  await page.route("**/api/trips/report", async (route) => {
    ...
    if (attempts === 1) {
      return route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          error: { code: "photo_rejected", message: "The photo was rejected." },
        }),
      });
    }
    return route.fulfill({
  ...
  await failed
    .getByRole("button", { name: "Retry report with this photo" })
    .click();
