...
const LOCAL_REVIEWER_A = "51111111-1111-4111-8111-111111111111";
const LOCAL_REVIEWER_B = "52222222-2222-4222-8222-222222222222";
const CHART_REVIEWER = "53333333-3333-4333-8333-333333333333";
...
async function loadSources() {
  const [
    ...
    readFile(new URL("field-review/san-mateo-structure-depth-review-policy.json", root)),
    ...
    readFile(new URL("docs/SAN-MATEO-STRUCTURE-DEPTH-REVIEW.md", root), "utf8"),
  ]);
  return {
...
function responseId(role, index) {
  const head = role === "local" ? "60000000" : "70000000";
...
function completeEvidence(sources) {
  const evidence = createSanMateoStructureDepthReviewTemplate({
    ...sources,
...
function evaluate(sources, evidence, evaluatedAt = EVALUATED_AT) {
  return evaluateSanMateoStructureDepthReview({
    ...sources,
    evidence,
...
test("the blank San Mateo policy is exact, source-bound, access-closed, and region-isolated", async () => {
  const sources = await loadSources();
  const result = validateSanMateoStructureDepthReview(sources);
  assert.deepEqual(result, {
    schemaVersion: "castingcompass.san-mateo-structure-depth-review/1.0.0",
  ...
  assert.throws(
    () => validateSanFranciscoStructureDepthReview(sources),
    /Expected the san-francisco structure\/depth review profile/u,
  );
  assert.deepEqual(
  ...
  assert.equal(sources.catalog.find(({ id }) => id === "pacifica-municipal-pier").accessStatus, "closed");
  assert.match(sources.guide, /remains closed[\s\S]+does\s+not reopen/iu);
});

test("complete disjoint San Mateo review emits only an aggregate non-authorizing receipt", async () => {
  const sources = await loadSources();
  const receipt = evaluate(sources, completeEvidence(sources));
  assert.equal(receipt.schema_version, "castingcompass.san-mateo-structure-depth-review-receipt/1.0.0");
...
test("missing, overlapping, stale, and correction-bearing San Mateo evidence fails closed", async () => {
  const sources = await loadSources();
  const blankReceipt = evaluate(
    sources,
    createSanMateoStructureDepthReviewTemplate({ ...sources, reviewedCommit: REVIEWED_COMMIT }),
  );
...
test("San Mateo digest, identity, unsafe-text, duplicate-ID, and artifact drift are rejected", async () => {
  const sources = await loadSources();
  ...
  unsafe.chart_responses[0].generalized_correction = "Email angler@example.com or inspect 37.60000, -122.50000";
  ...
  artifactDrift.sites["pacifica-municipal-pier"].scoreDelta = 1;
  assert.throws(
    () => validateSanMateoStructureDepthReview({ ...sources, artifact: artifactDrift }),
    /authority boundary/u,
  );
});

test("the San Mateo guarded writer creates one owner-only file and refuses overwrite", async () => {
  const sources = await loadSources();
  const directory = await mkdtemp(join(tmpdir(), "castingcompass-san-mateo-structure-review-"));
  ...
  try {
    ...
    assert.equal(receipt.schema_version, "castingcompass.san-mateo-structure-depth-review-template-write-receipt/1.0.0");
...
test("the San Mateo CLI has no provider client or process execution path", async () => {
  const source = await readFile(new URL("scripts/verify-san-mateo-structure-depth-review.mjs", root), "utf8");
