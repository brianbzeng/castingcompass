...
import {
  createSantaBarbaraStructureDepthReviewTemplate,
  evaluateSantaBarbaraStructureDepthReview,
  validateSantaBarbaraStructureDepthReview,
  writeSantaBarbaraStructureDepthReviewTemplate,
...
const LOCAL_REVIEWER_A = "11111111-1111-4111-8111-111111111111";
const LOCAL_REVIEWER_B = "22222222-2222-4222-8222-222222222222";
const CHART_REVIEWER = "33333333-3333-4333-8333-333333333333";
...
async function loadSources() {
  const [policySource, catalogSource, artifactSource, sourcePolicySource, sourceSnapshotSource, collectorSource, guide] = await Promise.all([
    readFile(new URL("field-review/santa-barbara-structure-depth-review-policy.json", root)),
    ...
    readFile(new URL("docs/SANTA-BARBARA-STRUCTURE-DEPTH-REVIEW.md", root), "utf8"),
  ]);
  return {
...
function responseId(role, index) {
  const head = role === "local" ? "10000000" : "20000000";
...
function completeEvidence(sources) {
  const evidence = createSantaBarbaraStructureDepthReviewTemplate({ ...sources, reviewedCommit: REVIEWED_COMMIT });
...
function evaluate(sources, evidence, evaluatedAt = EVALUATED_AT) {
  return evaluateSantaBarbaraStructureDepthReview({
    ...sources,
    evidence,
...
test("the blank Santa Barbara structure/depth policy is source-bound and non-authorizing", async () => {
  const sources = await loadSources();
  const result = validateSantaBarbaraStructureDepthReview(sources);
  assert.deepEqual(result, {
    schemaVersion: "castingcompass.santa-barbara-structure-depth-review/1.0.0",
  ...
  const template = createSantaBarbaraStructureDepthReviewTemplate({ ...sources, reviewedCommit: REVIEWED_COMMIT });
  assert.equal(template.local_responses.length, 0);
  assert.equal(template.chart_responses.length, 0);
  assert.equal(template.source_identity_recheck.reviewer_key, null);
  assert.equal(template.score_use_authorized, false);
  assert.equal(template.navigation_use_authorized, false);
});

test("complete disjoint local and chart review produces only an aggregate non-authorizing receipt", async () => {
  const sources = await loadSources();
  const evidence = completeEvidence(sources);
  const receipt = evaluate(sources, evidence);
  assert.equal(receipt.structure_depth_review_accepted, true);
  assert.equal(receipt.passing_site_count, 14);
  assert.equal(receipt.qualifying_local_response_count, 14);
  assert.equal(receipt.qualifying_chart_response_count, 14);
  assert.equal(receipt.distinct_local_reviewer_count, 2);
  assert.equal(receipt.distinct_chart_reviewer_count, 1);
  assert.equal(receipt.source_identity_recheck_current, true);
...
test("missing responses and source evidence fail closed with stable blockers", async () => {
  const sources = await loadSources();
  const evidence = createSantaBarbaraStructureDepthReviewTemplate({ ...sources, reviewedCommit: REVIEWED_COMMIT });
  const receipt = evaluate(sources, evidence);
...
test("reviewer-role overlap, stale evidence, and unresolved corrections remain blockers", async () => {
  const sources = await loadSources();
  ...
  const overlapReceipt = evaluate(sources, overlap);
  assert.equal(overlapReceipt.structure_depth_review_accepted, false);
  assert.equal(overlapReceipt.blockers.includes("reviewer-role-overlap"), true);
  ...
  correction.local_responses[0].question_answers.catalog_clue_fit = "correction_needed";
  correction.local_responses[0].correction_category = "clue";
  correction.local_responses[0].generalized_correction = "The generalized clue needs a less specific seasonal description.";
  const correctionReceipt = evaluate(sources, correction);
...
test("digest drift, extra fields, unsafe correction text, and duplicate identities are rejected", async () => {
  const sources = await loadSources();

  const driftedEvidence = completeEvidence(sources);
  driftedEvidence.structure_depth_artifact_sha256 = "0".repeat(64);
  assert.throws(() => evaluate(sources, driftedEvidence), /artifact digest drifted/u);
  ...
  unsafe.chart_responses[0].generalized_correction = "Email angler@example.com or inspect 34.42000, -119.70000";
...
test("artifact and catalog changes invalidate the committed review policy", async () => {
  const sources = await loadSources();

  const artifactDrift = structuredClone(sources.artifact);
  artifactDrift.sites["goleta-beach"].scoreDelta = 1;
  assert.throws(() => validateSantaBarbaraStructureDepthReview({ ...sources, artifact: artifactDrift }), /authority boundary/u);

  const clueDrift = structuredClone(sources.artifact);
  clueDrift.sites["goleta-beach"].structure.catalogClues[0].reviewStatus = "validated";
  assert.throws(() => validateSantaBarbaraStructureDepthReview({ ...sources, artifact: clueDrift }), /pre-validated/u);
});

test("the guarded writer creates one owner-only file and refuses overwrite", async () => {
  const sources = await loadSources();
  const directory = await mkdtemp(join(tmpdir(), "castingcompass-structure-review-"));
  await chmod(directory, 0o700);
  const outputFile = join(directory, "review.json");
  try {
    const receipt = await writeSantaBarbaraStructureDepthReviewTemplate({
...
test("the evaluator has no provider client or process execution path", async () => {
  const source = await readFile(new URL("scripts/verify-santa-barbara-structure-depth-review.mjs", root), "utf8");
