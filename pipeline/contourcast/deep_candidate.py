...
consume masked bags of single- or multiscale terrain patches and return the
same two-head interface as the classical candidates. It refuses real targets,
non-synthetic datasets, benchmark authority, locked-test access, winner
selection, score changes, serving changes, and deployment authority.
"""
...
DEEP_CAPABILITY_SCHEMA_VERSION = "castingcompass.deep-candidate-capability/1.0.0"
DEEP_CAPABILITY_STATUS = "synthetic-site-window-interface-smoke-only"
DEEP_CAPABILITY_EPOCHS = 4
DEEP_CAPABILITY_BASE_WIDTH = 8
DEEP_CAPABILITY_PATCHES_PER_WINDOW = 3
DEEP_CAPABILITY_SCALES = 2
DEEP_CAPABILITY_CHANNELS = 6
DEEP_CAPABILITY_PATCH_SIZE = 7
...
def _build_site_window_model(patch_bags: np.ndarray) -> Any:
    ...
    return AreaBagCatchModel(encoder, dropout=0.0)
...
def fit_predict_deep_candidate(
    ...
    *,
    ...
    """Fit the deep candidate on fictional patch bags under closed authority."""
    ...
    with _TORCH_CAPABILITY_LOCK, torch.random.fork_rng(devices=[]):
        ...
        try:
            ...
            for _ in range(DEEP_CAPABILITY_EPOCHS):
                outputs = model(train_tensor, train_mask_tensor)
...
def _synthetic_patch_fixture(
    *,
    ...
    """Build fictional multiscale site-window bags and complete labels."""
    ...
    return train_bags, occurrence, cpue, test_bags, train_mask, test_mask
...
def audit_synthetic_deep_candidate_capability(
    *,
    ...
    """Return a metric-free receipt for deterministic deep adapter plumbing."""
    ...
    train_bags, occurrence, cpue, test_bags, train_mask, test_mask = fixture
    ...
    return {
        ...
        "site_window_aggregation": "masked-attention-over-synthetic-patch-bags",
        ...
        "claim_boundary": (
            "Synthetic masked-patch interface and determinism smoke only; no "
