...
| `trips` | Account-owned trip time/site, effort, catch, gear, notes, moderation/AI state, and optional private photo locator | Until owner deletion, subject to pending-only edit/delete rules | Account deletion explicitly removes `WHERE user_id = authenticated user`; trip deletion binds both trip and owner. The user foreign key is `SET NULL` for database compatibility, so the explicit owner-scoped delete and its regression tests are mandatory |
