...
| `saved_sites` and `gear_profiles` | Account-owned preferences and gear presets | Until owner removes them or deletes the account | Owner predicates on ordinary routes; explicitly deleted in the account batch and backed by `ON DELETE CASCADE`. Gear creation returns `201` only after exact owner/content/timestamp read-back, including when the D1 insert response is lost |
