...
| `native_oauth_authorization_codes` | SHA-256 code hash, account/client/redirect, PKCE challenge, exact scope, issue/expiry, and one-use receipt | Five-minute authority window; bounded scheduled cleanup removes expired rows in batches of 100 | `ON DELETE CASCADE` from `users`; password reset removes outstanding codes; raw codes and verifiers never persist |
| `native_oauth_refresh_families` and `native_oauth_refresh_tokens` | Account/client/scope, family lifecycle, SHA-256 token hashes, generation, expiry, one-use rotation receipt, and compromise timestamp | Family lifetime is capped at 30 days; expired child rows then childless families are pruned in bounded batches | `ON DELETE CASCADE` from `users` through the family; reuse revokes the family and access tokens; password reset revokes families; account deletion cascades them |
| `native_oauth_access_tokens` | SHA-256 bearer hash, family/account/client/scope, issue/expiry, and revocation time | Ten-minute authority window; bounded scheduled cleanup removes expired rows in batches of 100 | `ON DELETE CASCADE` from both family and user; refresh rotation, family reuse, explicit revoke, password reset, and account deletion revoke or remove authority |
...
| Browser/native client state | Browser: HttpOnly session/receipt cookies, bounded anonymous reporter/age markers, local trip drafts, per-write trip recovery material, and ephemeral optional location. Native: ephemeral state/PKCE verifier and Keychain-only access/refresh tokens once implemented | Cookie/storage-specific bounded lifetimes; trip recovery material is removed after an exact receipt; location and PKCE attempt material are memory-only; native access is ten minutes and refresh family at most 30 days | Account deletion clears server authority even if client cleanup fails. Native credentials are forbidden from `UserDefaults`, files, app databases, pasteboard, logs, analytics, crash reports, URLs, and web `localStorage`; Keychain removal follows revoke/sign-out. Trip recovery material is not a session or administrator credential |
