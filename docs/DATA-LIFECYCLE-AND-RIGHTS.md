...
| `community_profiles`, `community_posts`, and `community_comments` | Account-owned pseudonymous handle/profile and place-linked user-generated discussion content with moderation state | Until owner deletion, moderation removal, or account deletion; published content remains public only while eligible | Each owner reference uses `ON DELETE CASCADE` from `users`; post deletion cascades to comments and related moderation rows. Privacy export includes owner content and moderation state but not account email as the public identity |
| `community_blocks` and `community_reports` | Owner block choices and abuse/privacy reports against community content | Until removed, resolved under moderation retention, or account deletion | Blocker and reporter references cascade from `users`; blocked-user and target-content deletion also removes dependent rows. Public projections never expose reports, block relationships, or account IDs |
| `community_moderation_queue` | Opaque post/comment review target, state, bounded assignment metadata, and timestamps | Until review resolution or target deletion; unresolved work remains queued for human moderation | Target deletion is enforced by migration triggers; account/content deletion cannot leave a queue item pointing at removed text. The queue is operator-only and is not returned by account or public APIs |
