# Santa Barbara structure and depth review packet
...
for the 14 Santa Barbara South Coast catalog locations. It does not ask anyone
to reveal a fishing spot or catch history, and it is not navigation advice,
access approval, a safety certification, or model validation.
...
npm run write:santa-barbara-structure-depth-review-template -- --output-file /absolute/private/path/structure-depth-review.json --expected-commit <full-commit-sha>
```
...
```sh
npm run evaluate:santa-barbara-structure-depth-review -- --evidence-file /absolute/private/path/structure-depth-review.json --expected-commit <full-commit-sha>
```
...
- `gaviota-state-park-beach` — **Gaviota State Park Beach**
- `refugio-state-beach` — **Refugio State Beach**
- `el-capitan-state-beach` — **El Capitán State Beach**
- `haskells-beach` — **Haskell's Beach**
- `goleta-beach` — **Goleta Beach**
- `arroyo-burro-beach` — **Arroyo Burro Beach**
- `mesa-lane-beach` — **Mesa Lane Steps Beach**
- `leadbetter-beach` — **Leadbetter Beach**
- `santa-barbara-harbor-breakwater` — **Santa Barbara Harbor Breakwater**
- `stearns-wharf` — **Stearns Wharf**
- `east-beach-santa-barbara` — **East Beach (Santa Barbara)**
- `summerland-beach-lookout-park` — **Summerland Beach (Lookout Park)**
- `carpinteria-state-beach` — **Carpinteria State Beach**
- `rincon-beach-park` — **Rincon Beach Park**
...
claim, or safety claim. Those gates remain separate and open.
