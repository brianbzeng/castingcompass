...
| `opportunities-browser.json` | Forecast pipeline; public browser projection of the canonical heuristic snapshot | Exact path plus the preserved generated/model identity inside the document; regenerated from `opportunities.json` | Browser revalidates; edge 5 minutes, stale-while-revalidate 10 minutes | New verified canonical snapshot/deployment or explicit purge | Network first, then the last verified PWA copy. The smaller projection removes browser-unused contract fields without changing scored windows; freshness logic must downgrade stale sources and must never invent missing values |
| `opportunities.json` | Forecast/data operations; canonical public heuristic snapshot retained for reproducibility and non-browser consumers | Exact path and complete scoring/snapshot identity inside the document | Browser revalidates; edge 5 minutes, stale-while-revalidate 10 minutes | New verified snapshot/deployment or explicit purge | Not fetched by the first-party browser UI. It remains the source used to regenerate the browser projection and preserves the complete model-run contract. |
