...
| Native account owner | Read the narrow current-profile projection with `profile:read`; start, cancel, complete/report, or manage an own pending trip with `trips:write` | Reuse browser cookies as native credentials, mix Cookie and Bearer authority, request undeclared scopes, submit an account role/ID, access export/photo/deletion/admin surfaces, or bypass current legal/deletion state | Default-off public-client authorization code with PKCE `S256`; exact client/redirect; five-minute one-use code; ten-minute access token; 30-day rotating refresh family; hash-only storage; family revocation on reuse; server-derived identity, route scopes, owner predicates, and no-cookie bearer parsing |
...
| Native authorization code | `code_hash = sha256(code)`, exact configured client/redirect/challenge/scope, unconsumed, unexpired, and owner not deletion-fenced | Browser-cookie session plus current legal acceptance may issue at most five active codes. Exchange compare-and-sets the complete one-use code snapshot before any token pair may escape; wrong verifier, replay, expiry, or ambiguous post-state grants nothing |
| Native refresh family/token | `token_hash = sha256(refresh)` joined to exact client, live family, scope, expiry, and account | Every refresh consumes one generation, revokes prior family access, and exactly reads back one successor pair. Reuse or ambiguous concurrency revokes the full family. Password reset, account deletion, or explicit revocation removes/revokes native authority |
| Native access token | `token_hash = sha256(bearer)`, exact client, live unrevoked family/token, expiry, declared route scope, current account/legal state, and no deletion fence | Accepted only with one strict Bearer header, no Cookie, and no browser Origin. Account identity is derived from the joined user row and bound into the same owner predicates as browser routes. Wrong-scope or malformed credentials never fall back to cookie authority |
