...
| Password reset | One-use verified challenge bound server-side to `email_challenges.user_id`; final credential update repeats that user ID | Enumeration-resistant request/resend responses remain identical for missing accounts and unconfirmed D1 work. The initial challenge INSERT atomically rechecks the five-per-hour email ceiling, and provider delivery follows only its one-row receipt. Candidate-only cleanup removes an ambiguous or undelivered version without deleting a newer concurrent code. The final password update and session revocation each repeat the exact verified challenge snapshot and claimed attempt count; consumption repeats it again. A concurrent resend changes the snapshot and leaves the password, sessions, and newer code untouched. Replacement session issuance requires authoritative receipts for all three atomic statements |
