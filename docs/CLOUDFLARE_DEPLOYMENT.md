...
Production fixes `workers_dev=false`, `preview_urls=false`,
`DEPLOYMENT_ENVIRONMENT=production`, and an empty `STAGING_HOSTNAME`. The first Worker
request branch admits only the canonical hostname and three reviewed redirect aliases; every
unknown, default, preview, or staging hostname receives a non-cacheable `421` before
redirects, rate limits, Turnstile, body reads, assets, or D1. Isolated staging uses a separate
Worker/configuration receipt and sets `DEPLOYMENT_ENVIRONMENT=staging` plus one exact HTTPS
hostname that must match its custom-domain route.

`worker-configuration.d.ts` is generated by the exact locked Wrangler version. CI runs
`wrangler types --check` and typechecks Worker code separately from browser code so the
Cloudflare runtime declarations cannot silently drift or replace DOM types.

HTML responses receive a fresh per-response script/style nonce and a complete
`Content-Security-Policy-Report-Only` policy at the Worker boundary. The existing minimal
enforced framing/object/base policy remains in place. Staging browser acceptance must capture
`securitypolicyviolation` events and reach zero unexplained violations before a later reviewed
change promotes the report-only policy to enforcement; do not replace the nonce mechanism with
component-specific hard-coded hashes.
