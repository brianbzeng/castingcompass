...
- persisted request descriptors that contain credential references and request hashes, never
  credential bytes;
...
The package contains no network scheduler and cannot silently replay a write. A future SwiftUI
application must materialize each exact request from Keychain-backed secrets, persist the
non-secret durable record with iOS file protection, and call the state machine only in response to
